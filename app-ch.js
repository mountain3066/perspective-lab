...
function drawSetupScene() {
  ...
  if (s0 && s1 && s2 && s3 && s0.visible && s1.visible && s2.visible && s3.visible) {
    ...
    if (layers.labels) {
      ...
      setupCtx.fillText('投影面 (画面)', s0.x + 5, s0.y + 15);
    }
  }
  ...
  if (sEye && sEye.visible) {
    ...
    if (layers.labels) {
      ...
      setupCtx.fillText('观察者 (视点)', sEye.x + 10, sEye.y - 5);
    }
  }
}
...
function drawPerspectiveScene() {
  ...
  if (layers.horizon) {
    ...
    if (layers.labels) {
      ...
      perspectiveCtx.fillText('地平线 (视平线)', 10, Y_horizon_pixel - 6);
    }
  }
  ...
  if (layers.vanishing) {
    const vpKeys = ['x', 'y', 'z'];
    const vpLabels = { x: 'Vx (X轴灭点)', y: 'Vy (Y轴灭点)', z: 'Vz (Z轴灭点)' };
      ...
      if (!vp.infinite) {
        ...
        if (onScreen) {
          ...
          if (layers.labels) {
            ...
            const labelStr = k === 'x' ? 'X 轴方向 ➔' : k === 'y' ? 'Y 轴方向 ➔' : 'Z 轴方向 ➔';
...
function updateExplainer() {
  ...
  if (isYawZero && isPitchZero) {
    badge.innerText = '一点透视';
    badge.className = 'explainer-mode-badge';
    badge.style.background = '#6366f1'; // Indigo
    title.innerText = '一点透视（平行透视）';
    text.innerText = '相机处于水平状态（俯仰角为 0°），且视线垂直于立方体的一个面（偏航角为 0° 或 90°）。此时仅存在一个灭点，位于视中心。画面中的水平线和垂直线仍与投影面保持平行，永不相交。';
  } else if (!isYawZero && isPitchZero) {
    badge.innerText = '两点透视';
    badge.className = 'explainer-mode-badge';
    badge.style.background = '#00d2ff'; // Cyan
    title.innerText = '两点透视（成角透视）';
    text.innerText = '相机处于水平状态（俯仰角为 0°），但绕着物体旋转了角度（偏航角非 0°）。地平线上会出现两个灭点（分别代表物体的宽度方向和深度方向的线条汇聚）。画面中的垂直线依然保持平行竖直。';
  } else {
    badge.innerText = '三点透视';
    badge.className = 'explainer-mode-badge';
    badge.style.background = '#ff5e62'; // Pink
    title.innerText = '三点透视（斜角透视）';
    text.innerText = '相机向上或向下倾斜（俯仰角非 0°），且绕着物体旋转。此时物体的三组平行线（宽度、高度、深度）均与投影面倾斜，各自收缩汇聚于独立的灭点（Vx、Vz 以及垂直方向的 Vy）。';
  }
}
